...
function ban4nft_dbreset_sqlite3()
{
    ...
    log_write($BAN4NFTD_CONF);
    // サービスを再起動する
    system('/usr/bin/ban4nftc restart');
}
?>
<?php
// ----------------------------------------------------------------------
...
function ban4nft_dbinit_sqlite3_count_db()
{
    ...
    // テーブルがなかったら作成
    $BAN4NFTD_CONF['count_db']->exec('CREATE TABLE IF NOT EXISTS count_tbl (address, service, registdate)');
    // インデックスを作成する
    $BAN4NFTD_CONF['count_db']->exec('CREATE INDEX IF NOT EXISTS count_idx ON count_tbl (address)');
    
    // journal_modeをWALにする
    $BAN4NFTD_CONF['count_db']->exec('PRAGMA journal_mode=WAL');
    // synchronousをNORMALにする
    $BAN4NFTD_CONF['count_db']->exec('PRAGMA synchronous=NORMAL');
...
function ban4nft_dbinit_sqlite3_ban_db()
{
    ...
    // テーブルがなかったら作成
    $BAN4NFTD_CONF['ban_db']->exec('CREATE TABLE IF NOT EXISTS ban_tbl (address, service, protcol, port, rule, unbandate)');
    // インデックスを作成する
    $BAN4NFTD_CONF['ban_db']->exec('CREATE INDEX IF NOT EXISTS ban_idx ON ban_tbl (address)');
    
    // journal_modeをWALにする
    $BAN4NFTD_CONF['ban_db']->exec('PRAGMA journal_mode=WAL');
    // synchronousをNORMALにする
    $BAN4NFTD_CONF['ban_db']->exec('PRAGMA synchronous=NORMAL');
...
function ban4nft_dbinit_sqlite3_mailrate_db()
{
    ...
    // テーブルがなかったら作成
    $BAN4NFTD_CONF['mailrate_db']->exec('CREATE TABLE IF NOT EXISTS mailrate_tbl (to_address, title, registdate, UNIQUE (to_address, title) )');
    // インデックスを作成する
    $BAN4NFTD_CONF['mailrate_db']->exec('CREATE INDEX IF NOT EXISTS mailrate_idx ON mailrate_tbl (to_address, title)');
    
    // journal_modeをWALにする
    $BAN4NFTD_CONF['mailrate_db']->exec('PRAGMA journal_mode=WAL');
    // synchronousをNORMALにする
    $BAN4NFTD_CONF['mailrate_db']->exec('PRAGMA synchronous=NORMAL');
