...
// Sub Routine
// ----------------------------------------------------------------------
function log_write($BAN4NFTD_CONF)
{
    // ログファイルの指定がないなら
    if (!isset($BAN4NFTD_CONF['log_file']) || !is_file($BAN4NFTD_CONF['log_file']))
    {
        print $BAN4NFTD_CONF['log_msg'];
        return;
    }
    ...
    {
        $BAN4NFTD_CONF['log_p'] = fopen($BAN4NFTD_CONF['log_file'], 'a');
    }
    // ログファイルポインタが開かれているなら
    if (is_resource($BAN4NFTD_CONF['log_p']))
    {
        // ログファイルを排他的ロック
        while(!flock($BAN4NFTD_CONF['log_p'], LOCK_EX));
        // ログファイルの一番最後までシーク
        fseek($BAN4NFTD_CONF['log_p'], 0, SEEK_END);
        // ログファイルに書き出し
        fprintf($BAN4NFTD_CONF['log_p'], $BAN4NFTD_CONF['log_msg']);
        // ログファイルをアンロック
        flock($BAN4NFTD_CONF['log_p'], LOCK_UN);
    }
    else
    {
        print $BAN4NFTD_CONF['log_msg'];
...
function ban4nft_dbinit_sqlite3_count_db($TARGET_CONF)
{
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR ".$TARGET_CONF['db_dir']." not found!?"."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR ".$TARGET_CONF['pdo_dsn_count']." not Connection!? ".$e->getMessage()."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit_sqlite3_ban_db($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR ".$TARGET_CONF['pdo_dsn_ban']." not Connection!? ".$e->getMessage()."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit_sqlite3_mailrate_db($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR ".$TARGET_CONF['pdo_dsn_mailrate']." not Connection!? ".$e->getMessage()."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit_pgsql_count_db($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit_pgsql_ban_db($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit_pgsql_mailrate_db($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit_mysql_count_db($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
    ...
    // インデックスが無いなら作成する
    $TARGET_CONF = ban4nft_db_exec($TARGET_CONF, 'count_db', 'ALTER TABLE count_tbl ADD INDEX count_idx (address)');
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit_mysql_ban_db($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
    ...
    // インデックスが無いなら作成する
    $TARGET_CONF = ban4nft_db_exec($TARGET_CONF, 'ban_db', 'ALTER TABLE ban_tbl ADD INDEX ban_idx (address)');
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit_mysql_mailrate_db($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
    ...
    // インデックスが無いなら作成する
    $TARGET_CONF = ban4nft_db_exec($TARGET_CONF, 'mailrate_db', 'ALTER TABLE mailrate_tbl ADD INDEX mailrate_idx (to_address, title)');
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_dbinit($TARGET_CONF)
{
    // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
    ...
    if (isset($TARGET_CONF['pdo_dsn_count']) && preg_match('/^pgsql/', $TARGET_CONF['pdo_dsn_count']))
    {
        // カウントデータベースの初期化処理
        // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
        $TARGET_CONF = ban4nft_dbinit_pgsql_count_db($TARGET_CONF);
    }
    ...
    else if (isset($TARGET_CONF['pdo_dsn_count']) && isset($TARGET_CONF['pdo_dsn_username']) && isset($TARGET_CONF['pdo_dsn_password']) && preg_match('/^mysql/', $TARGET_CONF['pdo_dsn_count']))
    {
        // カウントデータベースの初期化処理
        // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
        $TARGET_CONF = ban4nft_dbinit_mysql_count_db($TARGET_CONF);
    }
    ...
    else if (isset($TARGET_CONF['pdo_dsn_count']) && preg_match('/^sqlite/', $TARGET_CONF['pdo_dsn_count']))
    {
        // カウントデータベースの初期化処理
        // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
        $TARGET_CONF = ban4nft_dbinit_sqlite3_count_db($TARGET_CONF);
    }
    // カウントデータベースのデータソース名(DSN)の指定が上記以外なら
    else
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR ".$TARGET_CONF['pdo_dsn_count'].", not supported!? ".$e->getMessage()."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
    ...
    if (isset($TARGET_CONF['pdo_dsn_ban']) && preg_match('/^pgsql/', $TARGET_CONF['pdo_dsn_ban']))
    {
        // BANデータベースの初期化処理
        // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
        $TARGET_CONF = ban4nft_dbinit_pgsql_ban_db($TARGET_CONF);
    }
    ...
    else if (isset($TARGET_CONF['pdo_dsn_ban']) && isset($TARGET_CONF['pdo_dsn_username']) && isset($TARGET_CONF['pdo_dsn_password']) && preg_match('/^mysql/', $TARGET_CONF['pdo_dsn_ban']))
    {
        // BANデータベースの初期化処理
        // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
        $TARGET_CONF = ban4nft_dbinit_mysql_ban_db($TARGET_CONF);
    }
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR ".$TARGET_CONF['pdo_dsn_ban'].", not supported!? ".$e->getMessage()."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
    ...
    if (isset($TARGET_CONF['pdo_dsn_mailrate']) && preg_match('/^pgsql/', $TARGET_CONF['pdo_dsn_mailrate']))
    {
        ...
        // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
        $TARGET_CONF = ban4nft_dbinit_pgsql_mailrate_db($TARGET_CONF);
    }
    ...
    else if (isset($TARGET_CONF['pdo_dsn_mailrate']) && isset($TARGET_CONF['pdo_dsn_username']) && isset($TARGET_CONF['pdo_dsn_password']) && preg_match('/^mysql/', $TARGET_CONF['pdo_dsn_mailrate']))
    {
        // メール送信レートデータベースの初期化処理
        // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
        $TARGET_CONF = ban4nft_dbinit_mysql_mailrate_db($TARGET_CONF);
    }
    ...
    else if (isset($TARGET_CONF['pdo_dsn_mailrate']) && preg_match('/^sqlite/', $TARGET_CONF['pdo_dsn_mailrate']))
    {
        // メール送信レートデータベースの初期化処理
        // 2025.09.05 いつの間にかDBの初期化を親プロセスのみでするようになっていて処理構造がおかしくなっていたのでdbinitを見直し
       $TARGET_CONF = ban4nft_dbinit_sqlite3_mailrate_db($TARGET_CONF);
    }
    // メール送信レートデータベースのデータソース名(DSN)の指定が上記以外なら
    else
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR ".$TARGET_CONF['pdo_dsn_mailrate'].", not supported!? ".$e->getMessage()."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_issinit($TARGET_CONF)
{
    // 情報共有サーバー名が設定されていないなら
    if (!isset($TARGET_CONF['iss_server']))
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR iss_server is not set!?"."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR iss_username is not set!?"."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR iss_password is not set!?"."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR iss_get_time(".$TARGET_CONF['iss_get_time'].")"."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
    ...
    {
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ERROR iss_get_limit(".$TARGET_CONF['iss_get_limit'].")"."\n";
        // ログに出力する
        log_write($TARGET_CONF);
        // 終わり
        exit -1;
...
// ----------------------------------------------------------------------
// メイン設定ファイルがあるなら
if (is_file($BAN4NFTD_CONF['main_conf']))
{
    // メイン設定ファイルを読み込んで変数展開する
    $BAN4NFTD_CONF = parse_ini_file($BAN4NFTD_CONF['main_conf'], FALSE, INI_SCANNER_NORMAL);
}
// 無ければ、
else
{
    print $BAN4NFTD_CONF['main_conf']." not found!?"."\n";
    // 終わり
    exit -1;
}
// 読み込めなかったら
if ($BAN4NFTD_CONF === FALSE)
{
...
$BAN4NFTD_CONF['system_pid0'] = file_get_contents('/proc/1/comm');
$BAN4NFTD_CONF['system_pid0'] = rtrim($BAN4NFTD_CONF['system_pid0']);
?>
