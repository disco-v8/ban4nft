...
function ban4nft_ban($TARGET_CONF)
{
    ...
    {
        // メッセージを設定
        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", $TARGET_CONF['logtime'])." ban4nft[".getmypid()."]: NOTICE [".$TARGET_CONF['target_service']."] Ban ".$TARGET_CONF['target_address']." (This is ISS BAN Data, ".$TARGET_CONF['target_protcol']."/".$TARGET_CONF['target_port'].") ";
    }
    // 通常の監視によるBANなら
    else
    {
    ...
    if (isset($TARGET_CONF['target_rule']))
    {
        ...
        // BANした後のコマンド(exec_afer_ban)が設定されていたら実行(nftで設定するしないにかかわらず実行するように変更)
