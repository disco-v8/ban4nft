...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_close($TARGET_CONF)
{
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_loop($TARGET_CONF)
{
    // ログデータ初期化
    $LOGDATA = '';
    ...
    while (($EVENTS = inotify_read($TARGET_CONF['target_inotify'])) !== false)
    {
        ...
        foreach($EVENTS as $EVENT)
        {
            ...
            if ($EVENT['mask'] & IN_MOVE_SELF)
            {
                ...
                $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: "."INFO [".$TARGET_CONF['target_service']."] Stop. Target File was rotate? (".$TARGET_CONF['conf_file'].")"."\n";
                // 親プロセスに送信
...
////                                    $TARGET_CONF['logtime'] = local_time();
                                    ...
                                    if (check_safekeyword($TARGET_CONF) == TRUE)
                                    {
                                        ...
                                        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", $TARGET_CONF['logtime'])." ban4nft[".getmypid()."]: INFO [".$TARGET_CONF['target_service']."] Safe ".$TARGET_CONF['target_keyword']."\n";
                                        // 親プロセスに送信
                                        ban4nft_sendmsg($TARGET_CONF);
                                        // 次の対象文字列検査へ
                                        continue;
                                    ...
                                    $TARGET_CONF = ban4nft_db_exec($TARGET_CONF, 'count_db', "DELETE FROM count_tbl WHERE service = '".$TARGET_CONF['target_service']."' AND registdate < (".(time() - $TARGET_CONF['findtime']).")");
                                    // カウントデータベースにカウント対象キーワードを登録(IPアドレスじゃないのでクエリをエスケープする)
                                    $TARGET_CONF = ban4nft_db_exec($TARGET_CONF, 'count_db', "INSERT INTO count_tbl VALUES ('".$TARGET_CONF['target_keyword']."','".$TARGET_CONF['target_service']."',".$TARGET_CONF['logtime'].")");
                                    ...
                                    else
                                    {
                                        ...
                                        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", $TARGET_CONF['logtime'])." ban4nft[".getmypid()."]: INFO [".$TARGET_CONF['target_service']."] Found ".$TARGET_CONF['target_keyword']." (".$RESULT_COUNT."/".$TARGET_CONF['maxretry']." counts)"."\n";
                                        // 親プロセスに送信
                                        ban4nft_sendmsg($TARGET_CONF);
                                    }
                            }
...
////                                    $TARGET_CONF['logtime'] = local_time();
                                    ...
                                    $TARGET_CONF = ban4nft_db_exec($TARGET_CONF, 'count_db', "INSERT INTO count_tbl VALUES ('".$TARGET_CONF['target_address']."','".$TARGET_CONF['target_service']."',".$TARGET_CONF['logtime'].")");
                                    ...
                                    else
                                    {
                                        ...
                                        $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", $TARGET_CONF['logtime'])." ban4nft[".getmypid()."]: INFO [".$TARGET_CONF['target_service']."] Found ".$TARGET_CONF['target_address']." (".$RESULT_COUNT."/".$TARGET_CONF['maxretry']." counts)"."\n";
                                        // 親プロセスに送信
                                        ban4nft_sendmsg($TARGET_CONF);
                                    }
                                }
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_init($TARGET_CONF)
{
    // ログメッセージを初期化
    $TARGET_CONF['log_msg'] = '';
...
// Sub Routine
// ----------------------------------------------------------------------
function ban4nft_start($TARGET_CONF)
{
    do // loop_mode=1なら、ログ切りつめでも動作し続ける
    {
        // 対象ログの監視＆BAN処理の初期化
        $TARGET_CONF = ban4nft_init($TARGET_CONF);
        // 初期化に失敗したら
        if (strlen($TARGET_CONF['log_msg']))
        {
            ...
            foreach($TARGET_CONF['mail_to'] as $MAIL_TO)
            {
                ...
                date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ".$TARGET_CONF['log_msg'],
                'From: '.$TARGET_CONF['mail_from']."\n".
                $TARGET_CONF['mail_priority']."\n");
            }
            // メッセージを表示
            print date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: ".$TARGET_CONF['log_msg'];
            // 親プロセスに送信
            ban4nft_sendmsg($TARGET_CONF);
        ...
        {
            // 子プロセスがスタートした旨を表示
            $TARGET_CONF['log_msg'] = date("Y-m-d H:i:s", local_time())." ban4nft[".getmypid()."]: "."INFO [".$TARGET_CONF['target_service']."] Co-Process Start. (".$TARGET_CONF['conf_file'].")"."\n";
            // 親プロセスに送信
            ban4nft_sendmsg($TARGET_CONF);
            
            // 対象ログの監視＆BAN処理開始
            $TARGET_CONF = ban4nft_loop($TARGET_CONF);
        }
        // 対象ログの監視＆BAN処理の終了処理
        $TARGET_CONF = ban4nft_close($TARGET_CONF);
    }
